...
namespace WtPUpdater
{
	class WtpContainer
	{
		...
		internal IEnumerable<string> GetVerList()
		{
			...
			_addLog($"Getting release data from {_ghUri}");

			try
			{
				string result;
				_webClient.Headers[HttpRequestHeader.UserAgent] = "WtpUpdater/1.0";
				_webClient.Headers[HttpRequestHeader.Accept] = "application/vnd.github.v3+json";
				...
				_addLog($"Releases data length: {result?.Length ?? 0}");

				// Parse JSON with Newtonsoft.Json (Json.NET)
				...
				foreach (var release in releases)
				{
					...
					foreach (var asset in assets)
					{
						...
						const string prefix = "WeThePeople-";
						if (!name.StartsWith(prefix, StringComparison.OrdinalIgnoreCase)) continue;

						var version = name.Substring(prefix.Length).Replace(".zip", "");
						if (Blacklist.Contains(version)) continue;

						if (!_versionUris.ContainsKey(version))
						{
							_versionUris.Add(version, downloadUrl);
							versionList.Add(version);
							_addLog($"Found version {version} at {downloadUrl}");
						}
					}
				}
			}
